...
class PennyScraper(BaseScraper):
    """
    Scraper for Penny deals

    Penny is part of the REWE Group and publishes weekly deals
    on their website.
    """
    ...
    async def scrape_deals(self) -> List[Dict]:
        """Scrape current deals from Penny"""
        deals = []
        ...
        try:
            ...
            async with httpx.AsyncClient(
                ...
                if response.status_code == 403:
                    self.logger.warning("Penny: Bot protection detected (403)")
                    return deals

                if response.status_code != 200:
                    self.logger.warning(f"Penny: Failed to fetch (status {response.status_code})")
                    return deals

                soup = BeautifulSoup(response.text, 'html.parser')

                # Penny offer selectors
                deal_items = soup.select(
                    '.offer-tile, .product-tile, .bubble-offer, '
                    'article[class*="offer"], [class*="offer-card"]'
                )

                if not deal_items:
                    self.logger.info("Penny: No deal items found with primary selectors, trying fallback")
            ...
            self.logger.error(f"Error scraping Penny: {e}")

        self.logger.info(f"Penny: Found {len(deals)} deals")
        return deals
    ...
    def _parse_deal_item(self, item) -> Optional[Dict]:
        ...
        try:
            product_name = None
            for selector in [
                '.offer-tile__title', '.bubble-offer__title',
            ...
            discount_price = None
            for selector in [
                '.offer-tile__price', '.bubble-offer__price',
            ...
            self.logger.warning(f"Error parsing Penny deal item: {e}")
            return None
