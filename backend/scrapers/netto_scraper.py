...
class NettoScraper(BaseScraper):
    """
    Scraper for Netto Marken-Discount deals

    Netto publishes weekly deals on their website.
    """
    ...
    async def scrape_deals(self) -> List[Dict]:
        """Scrape current deals from Netto"""
        deals = []
        ...
        try:
            ...
            async with httpx.AsyncClient(
                ...
                if response.status_code == 403:
                    self.logger.warning("Netto: Bot protection detected (403)")
                    return deals

                if response.status_code != 200:
                    self.logger.warning(f"Netto: Failed to fetch (status {response.status_code})")
                    return deals

                soup = BeautifulSoup(response.text, 'html.parser')

                # Netto offer selectors
                deal_items = soup.select(
                    '.offer-tile, .product-tile, .product-card, '
                    'article[class*="offer"], [class*="angebot"]'
                )

                if not deal_items:
                    self.logger.info("Netto: No deal items found with primary selectors, trying fallback")
                    deal_items = soup.select('[class*="offer"], [class*="product"], article')
            ...
            self.logger.error(f"Error scraping Netto: {e}")

        self.logger.info(f"Netto: Found {len(deals)} deals")
        return deals
    ...
    def _parse_deal_item(self, item) -> Optional[Dict]:
        ...
        try:
            product_name = None
            for selector in [
                '.offer-tile__title', '.product-card__title',
            ...
            discount_price = None
            for selector in [
                '.offer-tile__price', '.product-card__price',
                '.price', '.price--current', '[class*="price"]'
            ]:
                element = item.select_one(selector)
                if element:
            ...
            self.logger.warning(f"Error parsing Netto deal item: {e}")
            return None
