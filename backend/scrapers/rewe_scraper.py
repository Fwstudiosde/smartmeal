...
class ReweScraper(BaseScraper):
    """
    Scraper for REWE deals

    REWE has a mobile API and a web offers page.
    This scraper attempts the public offers page first.
    """
    ...
    async def scrape_deals(self) -> List[Dict]:
        """Scrape current deals from REWE"""
        deals = []

        try:
            # Try REWE offers page
            url = f"{self.base_url}/angebote"
            ...
            async with httpx.AsyncClient(
                ...
                if response.status_code == 403:
                    self.logger.warning("REWE: Bot protection detected (403)")
                    return deals

                if response.status_code != 200:
                    self.logger.warning(f"REWE: Failed to fetch (status {response.status_code})")
                    return deals

                soup = BeautifulSoup(response.text, 'html.parser')

                # REWE offer card selectors
                ...
                if not deal_items:
                    self.logger.info("REWE: No deal items found with primary selectors, trying fallback")
                    deal_items = soup.select('[class*="offer"], [class*="product"], [class*="deal"]')
            ...
            self.logger.error(f"Error scraping REWE: {e}")

        self.logger.info(f"REWE: Found {len(deals)} deals")
        return deals
    ...
    def _parse_deal_item(self, item) -> Optional[Dict]:
        ...
        try:
            product_name = None
            for selector in [
                '.search-service-productTitle', '.offer-title',
            ...
            discount_price = None
            for selector in [
                '.search-service-productOffer__price',
                '.offer-price', '.price', '.price--current',
            ...
            original_price = None
            for selector in [
                '.search-service-productOffer__oldPrice',
                '.price--old', '.original-price', 'del', '.was-price'
            ]:
                element = item.select_one(selector)
                if element:
            ...
            self.logger.warning(f"Error parsing REWE deal item: {e}")
            return None
